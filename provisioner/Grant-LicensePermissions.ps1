...
$targetApps = @(
    ...
    @{ Name = "Claude IAM Agent Leaver";   ClientId = "LEAVER-APP-CLIENT-ID-000000000003" }
)

$permissionsNeeded = @("LicenseAssignment.ReadWrite.All")
...
# ── Step 4: Grant LicenseAssignment.ReadWrite.All to each target app ──
foreach ($app in $targetApps) {
    ...
    foreach ($perm in $permissionsNeeded) {
        ...
        if ($existingRoleIds -contains $roleId) {
            Write-Log ("  [SKIP] " + $perm + " already granted to " + $app.Name)
        } elseif (-not $WhatIf) {
            ...
            Write-Log ("  [GRANTED] " + $perm + " -> " + $app.Name) "ACTION"
        } else {
            Write-Log ("  Would grant: " + $perm + " -> " + $app.Name) "WHATIF"
        }
    }
}
...
Write-Log "Done -- LicenseAssignment.ReadWrite.All granted to all JML agents"
