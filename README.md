...
4. **Audit it** — [threat model](docs/threat-model.md) · [agent identity roadmap (now executed)](docs/agent-identity-roadmap.md) · [Electron hardening](docs/electron-security.md) · 130 automated tests enforced by the CI quality gate.
...
Download the `JML Console Setup` installer from the [latest GitHub Release](https://github.com/TrickyDanceMoves/jml-agent-fleet/releases/latest). No admin rights are required; it installs per-user to `%LOCALAPPDATA%\Programs\JML Console\`.
