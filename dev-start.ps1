<#
.SYNOPSIS
    Starts the full JML local dev stack: Azurite → Worker → API (Azure Functions)

.DESCRIPTION
    Opens three separate PowerShell windows:
...
Write-Host ""
Write-Host "All three services starting in separate windows." -ForegroundColor Green
Write-Host ""
Write-Host "Endpoints (once ready):" -ForegroundColor White
Write-Host "  POST http://localhost:7071/api/jml              (trigger JML event)"
Write-Host "  POST http://localhost:7071/api/webhooks/bamboohr (BambooHR webhook)"
